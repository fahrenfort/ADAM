function filter_cleanchansblinks_and_epoch(filepath,filenames,outpath, locutoff, hicutoff, start_epoch, end_epoch, varargin)
% function filter_eeg_and_epoch(filepath,filename,outpath, locutoff, hicutoff, start_epoch, end_epoch, varargin)
% reads in EEGLAB data, filters, removes and interpolates bad channels, epochs, removes blinks using gratton and coles, and writes out.
...
for filename = filenames
    ...
    EEG = clean_artifacts(EEG,'ChannelCriterion',.6,'Highpass','off','WindowCriterion','off','burst_crit','off');
    ...
    % remove eye blinks through pop_gratton
    channelnames = {EEG.chanlocs(:).labels};
    try
        eog_channels = select_channels(channelnames,'EOG');
        eeg_channels = select_channels(channelnames,'EEG');
    ...
    if eog_present
        for cChan = 1:numel(eog_channels)
            disp(['Removing blinks with the Gratton and Coles algorithm, using channel: ' channelnames{eog_channels(cChan)}]);
            EEG = pop_gratton(EEG, eog_channels(cChan));
        end
        % and remove ocular channels, as these will be flat now anyway
        EEG = pop_select(EEG, 'channel', eeg_channels);
    end
    % save data
