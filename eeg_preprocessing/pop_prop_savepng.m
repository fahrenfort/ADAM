...
function com = pop_prop_savepng(EEG, typecomp, chanorcomp, winhandle, spec_opt, savefile)
...
if length(chanorcomp) > 1
    for index = chanorcomp
        pop_prop_savepng(EEG, typecomp, index, 0, spec_opt, savefile);  % call recursively for each chanorcomp
    end;
	com = sprintf('pop_prop_savepng( %s, %d, [%s], NaN, %s, %s);', inputname(1), ...
                  typecomp, int2str(chanorcomp), vararg2str( { spec_opt } ), savefile);
    return;
end;
...
[dirpath,name] = fileparts(savefile);
if ~exist(dirpath,'dir')
    mkdir(dirpath);
end
set(fh,'Renderer','Painters');
saveas(fh,fullfile(dirpath,[name '_comp' num2str(chanorcomp) '.png']), 'png');
close all;
return;
