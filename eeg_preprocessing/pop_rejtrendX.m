...
%   >> [OUTEEG, rej, com] = pop_rejtrend( INEEG, typerej, elec_comp, ...
%                winsize, maxslope, minR, superpose, reject,calldisp,minNelec);
%   rej contains a logical scalar with the rejected trials
% 
...
%   minNelec   - A minimum number of these electrodes need to contain trends
%                for the trial to be rejected 
% 
% Outputs:
...
function [EEG, rej, com] = pop_rejtrendX( EEG, icacomp, elecrange, winsize, ...
				minslope, minstd, superpose, reject, calldisp, minNelec)
...
% hack: only reject trial if a minumum number of trials contain the
% artefact
rej = sum(tmprejE,1) >= minNelec;
% continue
