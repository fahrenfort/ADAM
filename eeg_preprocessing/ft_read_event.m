...
switch eventformat
  ...
  case {'biosemi_bdf', 'bham_bdf'}
    ...
    disp('MODIFICATIONS IN ft_read_event BY JOHANNES TO EXTRACT CORRECT TRIGGER CODES OF UU DATA, YOU MAY NEED TO COMMENT OUT OR IN ON LLINE 395:');
%     for c = 1:numel(event)
%         eventval = event(c).value;
%         if isnumeric(eventval) && ~isempty(eventval)
%             binVal = dec2bin(eventval);
%             numBits = size(binVal,2);
%             event(c).value = bin2dec(binVal(:,(numBits-7):numBits));
%         end
%     end
    % ENDS HERE
