...
function com = pop_topoplot_savepng( EEG, typeplot, arg2, topotitle, rowcols, varargin);
...
if nargin < 3
    ...
    if length(arg2) == 1, 
      curfig = figure; %figure('paperpositionmode', 'auto');
...
for index = 1:size(arg2(:),1)
	if nbgraph > 1
        if mod(index, rowcols(1)*rowcols(2)) == 1
            if index>1
                %figure(curfig); 
                a = textsc(0.5, 0.05, topotitle); set(a, 'fontweight', 'bold'); 
            end;
        	curfig = figure; 
            % set(curfig, 'visible', 'off');
            ...
            posx = 0;
            posy = 0;
            set(curfig, 'PaperUnits', 'centimeter');
            set(curfig, 'PaperPosition', [0 0 60 40]);
            %set(curfig, 'Position', [0 0 4*1024 4*768]); %x_width=10cm y_width=15cm
			% set(curfig,'PaperPosition', [posx posy  SIZEBOX*rowcols(2)  SIZEBOX*rowcols(1)]);
            %set(curfig, 'Position', [0, 0, 1024*2, 768*2]);
    ...
    if ~isnan(arg2(index))
		if typeplot
            ...
            %figure(curfig);  
            if nbgraph > 1, axes(curax); end; title(texttitle);
		end;
