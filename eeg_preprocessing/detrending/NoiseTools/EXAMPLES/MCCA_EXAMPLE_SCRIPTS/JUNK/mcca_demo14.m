...
disp('MCCA applied to real EEG data, with preprocessing');
...
if 0 
    figure(4);
    v=VideoReader('../parra_isc/SAR AV.mp4')
    ftf=zeros(6000,1);
    lastframe=readFrame(v);
    for iFrame=1:numel(ftf)
        if ~hasFrame(v); break; end
        frame=readFrame(v); 
        d=abs(frame-lastframe);
        ftf(iFrame)=mean(d(:));
        lastframe=frame;
        imagesc(frame); title(iFrame); drawnow
    end
    nframes=iFrame;
    ftf=ftf(1:nframes);
    teeg=(0:nsamples-1)'/sr;
    tvid=(0:nframes-1)'/v.FrameRate;
    ftf2=interp1(tvid,ftf,teeg);
    figure(5); 
    plot(ftf2)
end
