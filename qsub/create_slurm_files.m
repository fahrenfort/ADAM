function create_slurm_files(path_on_lisa, function_name, qsettings, varargin)
% create_slurm_files creates slurm files and bash files to submit slurm jobs for all subjects 
...
%
% Internal function of the ADAM toolbox by J.J.Fahrenfort, VU, 2014, 2015, 2016, 2018, 2019
%
...
outdir = varargin{3};
if use_scratch
    % timeout = 'timeout $timetorun '; % DOES NOT WORK ON SLURM
...
%             end
        ...
        elseif (mod(cQsubs,cores) == 0 && repeat == 1) || cores == 1 || cQsubs == size(combMat,1)
            ...
            if send_mail
                fprintf(fout,'echo "Job $SLURM_JOBID finished at `date`" | mail $USER -s "Job $SLURM_JOBID"\n');
            else
                fprintf(fout,'echo "Job $SLURM_JOBID finished at `date`"\n');
            end
            fclose(fout);
            fclose('all');
        end
...
for cqsubs = 1:numel(qsubfiles)
    do = ['sbatch ' qsubfiles{cqsubs}(max(strfind(qsubfiles{cqsubs},'/'))+1:end) '\n'];
    fprintf(fout,do);
