...
%   [shape]:   Shape of elements: 1 = square, 2 = circular, 3 = diamond (default: square), 4 = inducers
...
if shape == 4
    sizes = ones(1,15)*.05; % 10% of horizontal size
end
...
switch shape
    ...
    case 4 % read in shapes
        randinducers = randperm(6);
        for i = 1:length(sizes)
            template = 0*mask;
            tmp = imread(['/Users/VU-MBP/Dropbox/Work/Onderwijs/STAGES EN THESES/PSR/2017/paradigm/stimuli/illustrator/inducers/inducers_' num2str(randinducers(mod(i,6)+1)) '.png']);
            tmp = uint8(255) - tmp;
            tmp = imrotate(tmp,randi(360,1));
            tmp = crop_borders(tmp,tmp(1,1,:));
            relsize = sizes(i)/max(size(tmp));
            tmp = imresize(tmp,relsize);
            %inducerot = uint8(255) - inducerot;
            template(1:size(tmp,1),1:size(tmp,2),:) = tmp;
            sizes_templates{i} = find(template)-1;
        end
end
