...
% filename is name of EEG lab set file
% outpath is path to output file
% filename can also be an EEGlab structure or even a fieldtrip TFR
% structure. In that case, leave filepath empty.
% resample_to specifies the sampling rate to which to downsample. If left
% empty or specified as 0, no resampling is done.
% tf_and_erp_baseline is the period that is used as baseline normalization,
% in seconds e.g.: [-.4 -.15]. If this field is 0 or empty, no baseline
...
% baseline.
% You can specify the kind of TF baseline that is used by specifying
% method: e.g. method = 'absolute' (default). Other options: method =
% 'relative', 'relchange', 'normchange' or 'db'
% method is also used to specify the method(s) used for computing the TFR:
% method = 'total' (default), 'evoked' or 'induced'
% total and induced can either be used  with 'bin' or 'single', e.g.:
% method = 'relchange,induced,bin', meaning that trials will either be
% binned per condition (see below) or they will be kept as individual
% trials. Evoked is always the result of binning, otherwise there would be
% no evoked signal to compute the TFR on. Finally, induced can be computed
% using two methods: either by subtracting the average ERP from the bin class
% condition from each trial in that class (method = 'subtr_bin')
% or by subtracting the average ERP from the original condition label from
% each trial that has that label (method = 'subtr_indiv', default). TFR
...
% EVK_BIN: Computed evoked power values (averaging bins before power computation)
% INDC_SINGLE_SUBTR_BIN: Computed induced power per trial (each stimulus class' bin average erp subtracted from each respective trial before power computation)
% INDC_SINGLE_SUBTR_INDIV: Computed induced power values (each original condition label average erp subtracted from each respective trial before power computation) 
% INDC_BIN_SUBTR_BIN: Computed induced power values (each stimulus class' bin average erp subtracted from each respective trial before power computation, binning after power computation)
% INDC_BIN_SUBTR_INDIV: Computed induced power values (each original condition label average erp subtracted from each respective trial before power computation, binning after power computation)
...
% discarded 
% See compute_bins_on_FT_EEG for more details.
...
% use existing data, or import eeglab into ft
if isstruct(filename)
    FT_EEG = filename;
    [~,filename,~] = fileparts(tempname); % give it a random name
    if isfield(FT_EEG,'setname')
        FT_EEG = eeglab2ft(FT_EEG);
    end
else
    % remove .set at the end if present and import
    [~,filename,~] = fileparts(filename);
    FT_EEG = eeglab2ft(filename,filepath);
end
...
if strcmp(method,'induced')
    ...
    if binning
        ...
        fname = ['INDC_BIN_' upper(subtr_method) '_' filename];
    else
        fname = ['INDC_SINGLE_' upper(subtr_method) '_' filename];
    end
end
...
if strcmp(method,'total')
    ...
    if binning
        ...
        TFR = compute_bins_on_FT_EEG(TFR,condSet,'powspctrm','original');
        fname = ['TOT_BIN_' filename];
    else
        fname = ['TOT_SINGLE_' filename];
    end
end
